...
from emotions import C_CORE, C_WARM, C_PINK, C_PURPLE
...
class Lifeform:
    ...
    #    原因：
    #      1) 两条通道会叠加 —— 调到 50nit 时硬件 duty 25% × 软件 0.4 = 实际 10%，
    #         滑杆还没到底屏幕就黑了；
    #      2) 软件遮罩不省电（背光照样全亮），硬件 PWM 是真省电；
    #      3) RGB565 本来只有 5/6 bit，再乘 0.2 会把暖橙压成一坨色带。
    SOFT_DIM = False
