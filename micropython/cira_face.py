# -*- coding: utf-8 -*-
"""
CIRA 表情屏渲染（MicroPython）— cira_ 命名
============================================
极简「画布」抽象 + 两个后端（圆形 TFT / OLED），
face.draw_emotion() 只调用画布的通用图元，所以换屏不用改表情逻辑。

后端只需实现：fill(c) / hline(x,y,w,c) / vline(x,y,h,c) / line(x0,y0,x1,y1,c)
（颜色整数，单色屏自动把非零当亮）。

依赖：frozen `st77916`（圆形屏）；`cira_emotions`（调色板）。
本文件是「旧脸模型」的静态表情后备；主视觉由 cira_lifeform（光生命体星云）承担，
两者共用同一套画布图元，互不冲突。
"""

import math
from cira_emotions import spec
...
# 实际分辨率 360×360（官方 wiki 确认）；ST77916 走 QSPI（frozen 硬件驱动）
