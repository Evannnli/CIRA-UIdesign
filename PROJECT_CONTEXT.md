...
>
> **最后更新：2026-08-08** ｜ 对应产物：`cira-prototype` Web 原型 **v0.7**（已 git 入库 → `Evannnli/CIRA-UIdesign`）+ HARDWARE.md（硬件权威文档）+ HANDOFF v0.7 + MODULE_INTERFACES v0.7 同步 ｜ **Device Runtime `v0.8.4`（硬件目标 ESP32-S3-Touch-LCD-1.85C-BOX / V2）收尾：双击双播修复 + 设为开机固件（上电即 CIRA 光生命体）** ｜ 模块1/2 契约差异由「适配桥接层」兜住（MODULE_INTERFACES §10）
...
- ✅ **v0.8.4 收尾（2026-08-08）：双击双播修复 + 设为开机固件**。① **双击双播**：一次点按令 CST816 产生按下/移动/抬起多个中断，`was` 标志在抬手瞬间被清导致二次 `wake()` 把"哎！"和"我在。"都播；修复：`cira_wake.wake()` 加 2s 冷却（同一点触只播一个应答），`cira_main` 整个唤醒动作（应答+发模型）也加 2s 冷却门。② **卡死在红圈**：根因是 `cira_main.py` 从没被设成开机固件，板子上电跑原厂 `main.py`（红圈）且该固件自挂；本次把 `cira_main.py` 设为板子 `main.py`（原厂备份 `main_xiaozhi.py`，可逆），`mpremote reset` 后真机确认上电即跑 CIRA（星云+触摸+WiFi+WS 全起，无 traceback，未变砖）。推送修正：mpremote `fs cp` 多源只落最后一个文件，改单文件推送，14 个 `cira_*.py` 全在 flash；WiFi 超时 30s→15s。③ 设备已有 `subtitle_font.py`，中文全字库字幕可显示（缺字体降级分支保留作兜底）。**待 Evan 肉眼确认观感。**
