...
>
> **最后更新：2026-08-11** ｜ 对应产物：`cira-prototype` Web 原型 **v0.7** + HARDWARE.md + HANDOFF v0.7 + MODULE_INTERFACES v0.7 同步 ｜ **Device Runtime `v0.8.8`（路线 B 实验版）**：星云取消限帧+控制中心拖动局部刷新，零编译待验证，作路线 A 太重时的兜底 ｜ `v0.8.7` 仍是稳定基线 ｜ **路线 A（真 LVGL 固件）全链路打通 ✅**：lv_micropython + 自建 `cira_st77916` QSPI C 扩展已编译并烧录，探针 `tools/lvgl_hello.py` L0~L3 全绿（圆屏出 `CIRA · LVGL` 白字+圆环，L3 timer_handler 379 轮/3s 无卡顿）。下一步把 CIRA 实际 UI（控制中心/星云）迁到 LVGL 运行时（`cira_main_lvgl.py`）｜ 模块1/2 契约差异由「适配桥接层」兜住（MODULE_INTERFACES §10）｜ **实机已部署开机 splash**（`micropython/cira_splash.py`→板子 `:main.py`，通电即显示 `CIRA`+旋转环，不再黑屏）｜ **固件二进制归档 `device-builds/`（含 bootloader/partition-table）+ 版本索引 `VERSIONS.md`**；**CIRA-UIdesign 为唯一更新远端**，后续每次改动都 push 此仓库（原型版本与实机装机版本分开管理） ｜ **build4（DMA 异步崩板修复）为上一已验证实机版本；build5 黑屏复发根因已定位（idf.py 不转发 USER_C_MODULES → st77916 漏编），build6 用 make 包装重编含 st77916 模块、待无按键烧录+四色验证（`--before default_reset`）**
...
- ✅ **v0.9 LVGL 迁移启动（2026-08-09）**：Evan 反馈控制中心"一帧一帧"、星云"雷达图光点闪烁"，根因=纯 MicroPython 无 GPU/合成器/CSS 过渡（星云 `_interval=220ms`≈4.5fps 纯 Python 像素；控制中心整屏 `fill_rect` 重绘无脏区）。决策：**扒成熟系统 → LVGL（lv_micropython，同硬件 ESP32-S3）**，保持 Python、复用音频/WS/触摸/主循环，仅换显示+UI 层。本轮产出：`LVGL_MIGRATION.md`（计划/分阶段/中文困境/硬件验证清单）、`micropython/cira_lvgl_display.py`（ST77916↔LVGL flush，方案 A frozen / 降级 dummy）、`cira_lvgl_lifeform.py`（复用 v0.8.7 星云数学，渲染进 LVGL canvas，`tools/test_lvgl_lf_smoke.py` 六态 Mac 桩通过）、`cira_lvgl_control_center.py`（canvas 方案复用已验证像素绘制，保中文）、`cira_main_lvgl.py`（LVGL 运行时入口，与 `cira_main.py` 并存可回退）。**⏳ 待 Evan 本机：刷 lv_micropython → 跑 `tools/lvgl_hello.py` 探针 → 反馈现象（圆屏出字/黑屏/import 失败）→ 据此校正 API（v8/v9）与 ST77916 flush 方案 A/B → 刷 `cira_main_lvgl.py`。** 沙盒无 ESP-IDF/烧录链/ GitHub 凭据，代码已落盘待提交。
- ✅ **路线 A 全链路打通（2026-08-11 · 真机里程碑）**：lv_micropython 固件（含自建 `cira_st77916` QSPI C 扩展 `usermod_cira_st77916` + `usermod_lvgl` + `lvgl_interface`）已烧录，整片原厂已备份。探针 `tools/lvgl_hello.py` 软复位后**一次跑通 L0~L3 全绿**：L1 ST77916 直驱红/绿/蓝各 ~32ms（QSPI 链路通）；L2 `lv.display_create`+`set_draw_buffers`+`set_flush_cb` 真 flush → 圆屏出白字 `CIRA · LVGL` + 圆环（LVGL 渲染管线通）；L3 `timer_handler` 379 轮/3s 无卡顿。修掉 LVGL 9.6.0 三个运行时 API 坑（颜色格式 `lv.COLOR_FORMAT.RGB565` 命名空间 / 挂缓冲用 `set_draw_buffers` 而非不存在的 `set_draw_buf` / flush `color_p` 是 `lv.Pointer` 用 `__dereference__`）。⚠️ **关键排障教训：mpremote `exec`/`run` 不软复位，反复跑会堆污染致随机崩板，排障前先 `mpremote reset`**。视觉确认待 Evan 拿板子看红=红（若红显蓝 = RGB565 字节序反，优先 `cira_pins.LCD_MADCTL` 加 `0x08` BGR 位免重编）。下一步：把 CIRA 实际 UI 迁到 `cira_main_lvgl.py` 真机验证。
- ✅ **路线 A 实机开机 splash 部署 + 黑屏修复（2026-08-11）**：Evan 反馈通电后"带光的黑屏"。根因=板子开机只跑到 REPL、无程序自启（`mpremote run` 仅瞬态显示）。新增 `micropython/cira_splash.py`（仅依赖已验证的 `cira_lvgl_display`→lvgl/st77916，不碰音频/WS/触摸，极低崩板风险），部署为板子 `:main.py`，开机即显示持久 `CIRA`+旋转环。并给 `init_lvgl_display` 加**幂等保护**（同会话重复调用直接返回已建 display，避免探针与自启重复 init 堆出第二个 display 随机崩板）。固件三件套归档 `device-builds/`（firmware+bootloader+partition-table，带日期），新增 `VERSIONS.md` 版本索引（原型版本 / 实机装机版本分列），`device-builds/README.md` 写清烧录+装机命令。**CIRA-UIdesign 设为唯一更新远端**，后续改动都 push 此仓库。
- ✅ **路线 A build4 · DMA 异步崩板修复（2026-08-11）**：实机 splash 跑一段就 USB 掉线（静默看门狗重启）。根因=`esp_lcd_panel_io_tx_color` 颜色数据异步入队、不等 DMA 完成，旧版在 DMA 仍读源缓冲时就 `heap_caps_free`/覆盖 → 堆损坏 → 延迟 ISR 崩板。修复：注册 `on_color_trans_done` ISR 给二进制信号量，`blit_dma_safe()`/`fill()` 释放/覆盖前 `wait_dma_done()` 等 DMA 真完成；拷内部 SRAM 后 `esp_cache_msync` 清 D$。隔离测试确认：仅 on 存活、单 blit 存活、整屏 360 行 fill 旧版崩→修复后 `tools/minlvgl.py` 跑通 A→N 全绿 + `DONE`。固件重编为 **build4**（含信号量同步），**无按键烧录**（`--before default_reset`，USB-Serial/JTAG 模式，esptool 直连软复位）到板子；`flash_args-2026-08-11b.txt` + `*-2026-08-11b.bin` 三件套归档 `device-builds/`，`VERSIONS.md` 加 build4 行。`device-builds/README.md` 烧录命令已改为无按键。视觉确认待 Evan 拿板子看（红=红？红显蓝则 `cira_pins.LCD_MADCTL` 加 `0x08` BGR 位免重编）。
- 🐛 **build5 黑屏复发 + 根因定位（2026-08-11）**：build5 改用 `idf.py` **直编**（漏传 `USER_C_MODULES`）→ `st77916.c` 被 CMake 静默丢弃、根本没编进固件 → `import st77916` 失败 → `cira_lvgl_display` 降级 dummy flush → **屏全黑**（正是 Evan 反复遇到的"屏幕无法点亮"）。根因确诊：`idf.py` 直编时 env 的 `USER_C_MODULES` 进不了 CMake（`py/usermod.cmake:43` 读的是 **CMake 变量**；`py/py.mk:32` 读 Make 变量，只有 `make` 包装才把它转成 `-DUSER_C_MODULES=...` 注入）。**修法=一律用 make 包装重编**（见 MEMORY.md「构建命令铁律」）。本地 `tools/qpi_sim.py` 离线复刻 QPI 命令流 + ESP-IDF 字节反转，证明命令编码（opcode 居首字节 / COLMOD 0x3A / INVON 0x21 / SLPOUT 0x11 / DISPON 0x29 齐备）与 ST77916 QSPI 规范一致，排除命令层疑点。**build6 用 make 包装重编（含 st77916 模块），待无按键烧录 + 四色验证。**
- **【立即】LVGL 阶段0 探针（Evan 本机）**：刷 lv_micropython → `mpremote connect /dev/cu.usbmodem101 run tools/lvgl_hello.py` → 反馈现象 → 校正 API/flush → 刷 `cira_main_lvgl.py`。这是 v0.9 唯一的硬件阻塞点（沙盒无烧录链）。
