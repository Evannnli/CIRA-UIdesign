...
>
> **最后更新：2026-08-08** ｜ 对应产物：`cira-prototype` Web 原型 **v0.7**（已 git 入库 → `Evannnli/CIRA-UIdesign`）+ HARDWARE.md（硬件权威文档）+ HANDOFF v0.7 + MODULE_INTERFACES v0.7 同步 ｜ **Device Runtime `v0.8.5`（硬件目标 ESP32-S3-Touch-LCD-1.85C-BOX / V2）修复点按 NameError 崩溃 + 睡眠改微暗呼吸（不再全黑）** ｜ 模块1/2 契约差异由「适配桥接层」兜住（MODULE_INTERFACES §10）
