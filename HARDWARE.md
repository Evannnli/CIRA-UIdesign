...
- **硬件版本**：该板 **V1 已于 2026-01-30 停产**，之后出厂为 **V2**。但 BOX 的固件仍分 V1/V2 两版（小智固件列表明示），**故你的实体板可能是 HW V1 或 V2**——两者音频方案与部分 GPIO 不同（见 §10）。**务必先确认你的板子是 V1 还是 V2（看 PCB 丝印/外壳标签）**，否则音频移植会整体错配。
- 本文件**默认按 V2 编写**（2026-08 新板大概率 V2），但凡 V2 专属项均标注「V2」，V1 差异见 §10。
...
1. **【最关键】你的板是 HW V1 还是 V2？**
   - 看 PCB 丝印或外壳标签（V2 标签）。今天 2026-08 新板大概率 V2，但不能假设。
   - 影响：音频方案（ES8311+ES7210 vs PCM5101A）、I2S 引脚、GPIO2/10/11/15 功能。错判 = 音频全错且难查。
...
- **Device Runtime 目标版本：`v0.8.0`**（硬件目标基线，对应本文件锁定的 ESP32-S3-Touch-LCD-1.85C-BOX / 假设 V2）。
