...
> 版本：v0.5 ｜ 日期：2026-08-07 ｜ 适用：360×360 圆形屏设备（ESP32 + 圆形 LCD/AMOLED）
...
stateDiagram-v2
    ...
    IDLE --> LISTENING: 语音唤醒("你好，cira"/"你好，西拉")
    IDLE --> SETTINGS: 长按 1.2s
    IDLE --> OFFLINE: 网络断开
    IDLE --> SLEEP: 空闲超时(默认 30s,待产品定)
    LISTENING --> THINKING: ASR 识别完成
    LISTENING --> SLEEP: ASR 超时未识别(默认 60s)
    THINKING --> SPEAKING: 模型生成回复
    THINKING --> SLEEP: 模型超时无响应(默认 15s)
    SPEAKING --> IDLE: TTS 播报结束(1.5s 后)
    SETTINGS --> IDLE: 点「完成」或退出
    OFFLINE --> IDLE: 网络恢复
    SLEEP --> IDLE: 触摸唤醒(任意位置)
    SLEEP --> LISTENING: 语音唤醒(跳过 IDLE 直入聆听)
```
...
- **交互**：
  ...
  - **语音唤醒（唤醒方式之二）**：待机（IDLE）时设备处于「语音待命」，底部显示麦克风按钮（呼吸光环）+「在听·唤醒词」气泡，被动聆听唤醒词 **“你好，cira”（英文发音）/ “你好，西拉”（中文发音）**；命中即进入 LISTENING，与点按同路径。
    - 麦克风按钮可点按 **开/关** 语音唤醒（默认开）。
    - 不支持语音识别的浏览器（无 Web Speech API）降级为「点话筒模拟唤醒」，保证交互可演示。
...
**触发时机（进入 SLEEP）**：
- `IDLE` 空闲超时（默认 **30 s**，待产品定）→ SLEEP
- `LISTENING` ASR 超时未识别（默认 **60 s**）→ SLEEP
- `THINKING` 模型超时无响应（默认 **15 s**）→ SLEEP
- `SPEAKING` 播报结束 → IDLE → 30 s → SLEEP（两级超时，避免立刻灭屏让用户感到突兀）

**唤醒路径（离开 SLEEP）**：
- **触摸唤醒**：任意位置触摸 → 直接进 `IDLE`，**不进 LISTENING**（避免误触发对话）
- **语音唤醒**：唤醒词命中 → **跳过 IDLE 直入 LISTENING**（用户开口就是要对话）
- **设置快捷入口**：保留"长按 1.2s 进 Settings"在 SLEEP 唤醒后第一帧就生效（即 IDLE 状态下长按照常）
...
| 事件/数据 | 字段 | 触发 |
|---|---|---|
| 唤醒 | `wake` | 用户点按 或 唤醒词命中（端侧唤醒词引擎） |
| **进入睡眠** | `enter_sleep` | 设备主动发起，告知模型停止收发，进入低功耗 |
| **离开睡眠** | `leave_sleep {reason: 'touch'\|'voice'}` | 触摸/语音唤醒后告知模型，reason=voice 时模型立刻准备收声 |
...
6. **SLEEP 超时参数（v0.5 新增）**：以下默认值为产品手感取舍，待你和产品确认。
   - IDLE → SLEEP：**30 s**（默认）
   - LISTENING → SLEEP（ASR 超时）：**60 s**
   - THINKING → SLEEP（模型超时）：**15 s**
   - SPEAKING → IDLE → SLEEP：播报结束后 **30 s**
