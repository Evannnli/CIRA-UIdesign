...
> 版本：v0.7 ｜ 日期：2026-08-08 ｜ 适用：360×360 圆形屏设备（ESP32 + 圆形 LCD/AMOLED）
...
- **v0.7** 模块化架构落地（Evan 决策：CIRA 采用模块化，1/2 由其他团队在 GitHub 维护，本仓库专攻模块3 Device Runtime）：① 对话流重构为「模块1 `CIRA.Core.respond()` → 模块2 `CIRA.Language.synthesize()` → AudioHandle 播放 + 星云动画」，唤醒打断经 `AudioHandle.stop()`；② 新增 `modules.js` 作为 Device Runtime 访问 1/2 的**唯一集成接缝**（本地占位桩 ↔ 真实远程模块只改 `CIRA_INTEGRATION` 配置，app.js 不动）；③ `core.js`/`language.js` 明确为**本地占位桩**（非真模块）；④ 新增 `MODULE_INTERFACES.md`（冻结接口契约 + **§7 模块3 需要你提供什么**：模块1/2 的 GitHub/版本/传输/端点/鉴权 + 硬件设备信息 + 烧录环境）。版本戳 `?v=0.7`。
